...
Page({
    ...
    lookAccredit: (e) => {
        wx.getSetting({
            success: function(res) {
                console.log('res', app.globalData.nickName, app.globalData.mobile)
    ...
    userSaveFn: function() {
        ...
        let nickname = _this.data.name || _this.data.nickName1;
        ...
        utils.uPost(`${api.HOST}/api/user/${app.globalData.userId}/update`, data).then((res) => {
            utils.successShow('修改成功').then(() => {
                utils.uGet(api.getUser, {}, true).then((res) => {
                    if (res) {

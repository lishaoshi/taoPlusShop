...
Page({
  ...
  onLoad: function(options) {
    _this = this;
    console.log('submit_orders_options:');
    console.log(options);
    ...
    utils.wxLogin().then(() => {
      ...
      _this.phoneNumFn();
      _this.getShopInfoFn();
      console.log('shopID:::', _this.data.shopId)
  ...
  submitFn: () => {
    ...
    utils.uPost(url, formData).then((res) => {
      ...
      wx.navigateTo({
        url: `../pay/pay?money=${sum}&goodsName=${_this.data.info.goods_name}&orderId=${res.order_id}&shopId=${_this.data.shopId}&grouponJson=${grouponJson}&type=${_this.data.type}&shopName=${_this.data.shopName}`
      })
